...
class DirichletBC:
    ...
    def apply(self, model, x_boundary, u_boundary, n_out, u_pred_var_list, u_train_var_list, space_dim=None,
              x_boundary_sym=None, boundary=None, vel_wave=None):
...
class NeumannBC:
    ...
    def apply(self, model, x_boundary, u_boundary, n_out, u_pred_var_list, u_train_var_list, space_dim=None,
        ...
        u_boundary_pred = model(x_boundary)
        grad_u = torch.autograd.grad(u_boundary_pred, x_boundary, grad_outputs=torch.ones_like(u_boundary_pred),
                                     create_graph=True)[0]
        grad_u_t = grad_u[:, 0]
