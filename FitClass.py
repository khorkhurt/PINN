...
class CustomLoss(torch.nn.Module):
    ...
    def forward(self, Ec, network, x_u_train, u_train, x_b_train, u_b_train, x_f_train, dataclass):
        ...
        max_u = torch.max(u)
        min_u = torch.min(u)
        # loss_1 = torch.maximum(max_u - 1.1, torch.tensor(0))
        # loss_2 = torch.maximum(-min_u, torch.tensor(0))
        ...
        loss_v = torch.log10(lambda_residual*loss_vars + lambda_pde * loss_res + lambda_reg * loss_reg)
        print("Total Loss:", loss_v.detach().cpu().numpy().round(4),
              ...
              # "| Loss 1:", loss_1.detach().cpu().numpy().round(4),
              # "| Loss 2:", loss_2.detach().cpu().numpy().round(4),
