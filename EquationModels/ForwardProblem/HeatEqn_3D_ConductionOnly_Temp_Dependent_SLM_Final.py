...
class EquationClass(EquationBaseClass):
    ...
    def __init__(self, ):
        ...
        self.type_of_points = "sobol_center_3d_moving"
        ...
        # The dimension of the domain, the number of space coordinates, for 2D case 2, for 3D case 3.
        ...
        self.norm_y = 1
        # Normalization in the z direction
        self.norm_z = 1
        ...
        self.list_of_BC = list([[self.ub0, self.ub0], [self.ub0, self.ub0], [self.ub0, self.ub0]])
    ...
    def compute_res(self, network, x_f_train, solid_object):
        ...
        Y = np.array(y_t, dtype=np.float32)
        y_t = torch.from_numpy(y_t)
        z_t = node_coords_numpy[:, 2].reshape(-1, 1)
        Z = np.array(z_t, dtype=np.float32)
        z_t = torch.from_numpy(z_t)
        # z_t = torch.full(size=(x_t.shape[0], 1), fill_value=0.03)
        

        x = torch.reshape(torch.linspace(-0.07, 2.07, 500), [500, 1])
        y = torch.reshape(torch.linspace(-0.075, 0.215, 200), [200, 1])
        X = x.numpy()
        Y = y.numpy()
        X, Y = np.meshgrid(X, Y)
        x_t = torch.reshape(torch.from_numpy(X), [100000, 1])
        y_t = torch.reshape(torch.from_numpy(Y), [100000, 1])
        z_t = torch.full(size=(x_t.shape[0], 1), fill_value=0.03)


        for t in range(len(times)):
            t = times[t, 0] / self.norm_t
            plot_var1 = torch.cat([torch.tensor(()).new_full(size=(x_t.shape[0], 1), fill_value=t), x_t, y_t, z_t], 1)
            torch.set_printoptions(profile="full")
            q = self.source(plot_var1)
            q = q.detach().numpy()
            np.savetxt("q.csv", q)
            fig, ax = plt.subplots(ncols=1, figsize=(75, 10))
            scatter_plot = ax.scatter(X, Y, c=q)
            ax.set_title(r'q', fontsize=72)
            ax.set_xlabel(r'x', fontsize=72)
            ax.set_ylabel(r'y', fontsize=72)
            ax.set_xticks([])
            ax.set_yticks([])
            legend = ax.legend(*scatter_plot.legend_elements(),
                                loc="lower left", title="q=", fontsize=32)
            plt.setp(legend.get_title(), fontsize=32)
            ax.add_artist(legend)
            images_path = "C:/Users/yasa/Desktop/PinnsLV_v3/PinnsLV/" \
                          "HeatEqn_3D_ConductionOnly_Temp_Independent_SLM/Images"
        ...
        grad_u_zz = torch.autograd.grad(grad_u_z, x_f_train, grad_outputs=torch.ones_like(u),
        ...
        source = q * self.norm_t / (self.rho * self.cp * self.norm_temp)
    ...
    def source(self, x):
        ...
        c = 0.04
        V = 2 / 0.002
        heat = (6 * np.sqrt(3) * f * Q / (a * b * c * pi * np.sqrt(pi))) * \
               (torch.exp(-3 * (((x[:, 1] * self.norm_x - V * x[:, 0]) / c) ** 2 +
                                (((x[:, 2] * self.norm_y) / a) ** 2) +
                                ((((x[:, 3] * self.norm_z) - 0.03) / b) ** 2))))
    ...
    def plotting(self, model, val_path, images_path, extrema):
        ...
        for t in range(len(times)):
            ...
            fig, ax = plt.subplots(ncols=1, figsize=(75, 10))
            scatter_plot = ax.scatter(X, Y, c=Temp_FE)
            ax.set_title("FE Result at t=%f s" % (times[t, 0]), fontsize=72)
            ax.set_xlabel(r'x', fontsize=72)
            ax.set_ylabel(r'y', fontsize=72)
            ax.set_xticks([])
            ax.set_yticks([])
            ...
            legend = ax.legend(*scatter_plot.legend_elements(),
                               loc="lower left", title="T=", fontsize=32)
            plt.setp(legend.get_title(), fontsize=32)
            ax.add_artist(legend)
        ...
        for t in range(len(times)):
            t_value = times[t, 0] / self.norm_t
            plot_var1 = torch.cat([torch.tensor(()).new_full(size=(x_t.shape[0], 1), fill_value=t_value), x_t, y_t,
                                   z_t], 1)
            plot_var_final = torch.cat([torch.tensor(()).new_full(size=(x_t.shape[0], 1), fill_value=1), x_t, y_t,
                                        z_t], 1)
            torch.set_printoptions(profile="full")

            output = model(plot_var1.float()) * self.norm_temp
            output = output.detach().numpy()

            fig, ax = plt.subplots(ncols=1, figsize=(75, 10))
            scatter_plot = ax.scatter(X, Y, c=output)
            ax.set_title("PINN Prediction at t=%f s" % (t_value * self.norm_t), fontsize=72)
            ax.set_xlabel(r'x', fontsize=72)
            ax.set_ylabel(r'y', fontsize=72)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.text(2, 95, "Tmax = %f" % (np.amax(output)), fontsize=32, color='white')
            legend = ax.legend(*scatter_plot.legend_elements(),
                               loc="lower left", title="T=", fontsize=32)
            plt.setp(legend.get_title(), fontsize=32)
            ax.add_artist(legend)
