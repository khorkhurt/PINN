...
class EquationClass(EquationBaseClass):
    ...
    def compute_res(self, network, x_f_train, solid_object):
        ...
        residual = grad_u_t.reshape(-1, ) - (self.k / self.rho / self.cp) * (grad_u_xx.reshape(-1, ) / (self.norm_x ** 2) +
                                                                        grad_u_yy.reshape(-1, ) / (
    ...
    def source(self, x):
        heat = (torch.exp(-((((x[:, 1] * self.norm_x - (0.1))/0.01).double() ** 2) +
                            (((x[:, 2] * self.norm_y - (0.1))/0.01).double() ** 2)))) * (10 ** 9)
        return heat
