...
class CustomLoss(torch.nn.Module):
    ...
    def forward(self, Ec, network, x_u_train, u_train, x_b_train, u_b_train, x_f_train, dataclass):
        ...
        #                       abs(u_pred_ini_vars - u_train_ini_vars) ** 4))
        loss_ini = (torch.mean(abs(u_pred_ini_vars - u_train_ini_vars) ** 2))
        # loss_ini = (torch.mean(abs(u_pred_ini_vars - u_train_ini_vars) ** 2))/(torch.mean(abs(u_train_ini_vars) ** 2))
        # loss_bound = (torch.mean(abs(u_pred_bound_vars - u_train_bound_vars) ** 2))/(torch.mean(abs(u_train_bound_vars) ** 2))
        '''
        initial_loss = abs(u_pred_ini_vars - u_train_ini_vars) ** 2 + abs(u_pred_ini_vars - u_train_ini_vars) ** 4
        loss_new_initial = (torch.mean((0.1 + 0.9*torch.exp(-((x_u_train[:, 0]-0.025)*500)**2)) * initial_loss))
        '''
        np.savetxt("x_f_train.csv", x_f_train.detach().numpy())
        res = Ec.compute_res(network, x_f_train, None).to(Ec.device)
        # heat_balance = Ec.heat_balance(network).to(Ec.device)

        loss_res = (torch.mean(abs(res) ** 2))
        loss_vars = loss_ini + loss_bound
        # loss_vars = loss_ini
        # loss_vars = loss_new_initial + loss_bound
        ...
        print("Total Loss:", loss_v.detach().cpu().numpy().round(4),
              "| Boundary Loss:", torch.log10(loss_bound).detach().cpu().numpy().round(4),
              "| Initial Loss:", torch.log10(loss_ini).detach().cpu().numpy().round(4),
              "| PDE Loss:", torch.log10(loss_res).detach().cpu().numpy().round(4), "\n")
