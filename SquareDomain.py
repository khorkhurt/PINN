...
class SquareDomain:
    def __init__(self, output_dimension, time_dimensions, space_dimensions, list_of_BC, extrema_values, type_of_points,
                 parameters_values=None, vel_wave=None):
        print(parameters_values)
        self.output_dimension = output_dimension
        self.time_dimensions = time_dimensions
        self.space_dimensions = space_dimensions
        self.list_of_BC = list_of_BC
        self.extrema_values = extrema_values if parameters_values is None else torch.cat([extrema_values,
                                                                                          parameters_values], 0)
    ...
    def add_boundary_points(self, n_boundary, random_seed):
        ...
        for i in range(self.time_dimensions, self.time_dimensions + self.space_dimensions):
            ...
            x_boundary_0[:, i] = torch.full(size=(x_boundary_0.shape[0],), fill_value=0.0)
            x_boundary_0_wo_i = np.delete(x_boundary_0, i, 1)

            [y_boundary_0, type_BC] = self.list_of_BC[i - self.time_dimensions][0](x_boundary_0_wo_i *
                                                                                   (val_f - val_0) + val_0)
            ...
            x_boundary_1[:, i] = torch.tensor(()).new_full(size=(x_boundary_1.shape[0],), fill_value=1.0)
    ...
    def apply_boundary_conditions(self, model, x_b_train, u_b_train, u_pred_var_list, u_train_var_list):
        ...
        for j in range(self.output_dimension):
            for i in range(self.space_dimensions):
                ...
                x_b_train_i = x_b_train[i * int(x_b_train.shape[0] /
                                                self.space_dimensions):(i + 1) * int(x_b_train.shape[0] /
                                                                                     self.space_dimensions), :]
                u_b_train_i = u_b_train[i * int(x_b_train.shape[0] /
                                                self.space_dimensions):(i + 1) * int(x_b_train.shape[0] /
                                                                                     self.space_dimensions), :]

                boundary = 0
                while boundary < 2:
                    x_b_train_i_half = x_b_train_i[half_len_x_b_train_i * boundary:half_len_x_b_train_i *
                                                                                   (boundary + 1), :]
                    u_b_train_i_half = u_b_train_i[half_len_x_b_train_i * boundary:half_len_x_b_train_i *
                                                                                   (boundary + 1), :]

                    x_half_1 = x_b_train_i_half
                    x_half_2 = x_b_train_i[half_len_x_b_train_i * (boundary + 1):half_len_x_b_train_i *
                                                                                 (boundary + 2), :]

                    boundary_conditions = self.BC[i][boundary][j]
                    boundary = boundary_conditions.apply(model, x_half_1, u_b_train_i_half, j, u_pred_var_list,
                                                         u_train_var_list, space_dim=i,  x_boundary_sym=x_half_2,
                                                         boundary=boundary, vel_wave=self.vel_wave)
