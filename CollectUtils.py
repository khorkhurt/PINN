...
def select_over_retrainings(folder_path, selection="error_train", mode="min", compute_std=False, compute_val=False,
                            rs_val=0):
    Ec = EquationClass()
    ...
    for retraining in retrain_models:
        ...
        if os.path.isfile(retrain_path + "/InfoModel.txt"):
            ...
            if os.path.isfile(retrain_path + "/TrainedModel/model.pkl"):
                ...
                trained_model.eval()
                test_inp = Ec.convert(torch.rand([100000, Ec.extrema_values.shape[0]]), Ec.extrema_values)
                Exact = (Ec.a_ex(test_inp)).numpy().reshape(-1, 1) ** 2
                test_out = abs(trained_model(test_inp)[:, -1].reshape(-1, 1)).detach().numpy()
                ...
                models["a_rel"] = rel_L2_test'''
                # print(models)
                if compute_std:
                    inp_bound_0 = np.concatenate([np.random.uniform(t_0, t_f, [2000, 1]), np.full((2000, 1), x_0)],
                                                 axis=1)
                    inp_bound_1 = np.concatenate([np.random.uniform(t_0, t_f, [2000, 1]), np.full((2000, 1), x_f)],
                                                 axis=1)


                    inp_bound = torch.from_numpy(np.concatenate([inp_bound_0, inp_bound_1], 0)).type(torch.FloatTensor)
                    inp_initial = torch.from_numpy(np.concatenate([np.full((2000, 1), t_0),
                                                                   np.random.uniform(x_0, x_f, [2000, 1])],
                    ...
                    # inp = torch.from_numpy(np.concatenate([inp_bound_0, inp_bound_1, inp_initial], axis=0)).
                    ...
                    inp_res = inp_res * (np.max(extrema_values, axis=1) - np.min(extrema_values, axis=1)) + \
                              np.min(extrema_values, axis=1)
                    inp_res = torch.from_numpy(inp_res).type(torch.FloatTensor)
                    residuals = compute_residual(trained_model, inp_res).detach().numpy()
                    sigma_Ru0 = np.std((exact(inp_initial).detach().numpy() -
                                        trained_model(inp_initial).detach().numpy()) ** 2)
                    sigma_Rub0 = np.std((exact(inp_bound_0).detach().numpy() -
                                         trained_model(inp_bound_0).detach().numpy()) ** 2)
                    sigma_Rub1 = np.std((exact(inp_bound_1).detach().numpy() -
                                         trained_model(inp_bound_1).detach().numpy()) ** 2)
                    ...
                    der_0_pred = torch.autograd.grad(ub0_pred, inp_bound_0, grad_outputs=torch.ones_like(ub0_pred),
                                                     create_graph=True)[0]
                    der_1_pred = torch.autograd.grad(ub1_pred, inp_bound_1, grad_outputs=torch.ones_like(ub0_pred),
                                                     create_graph=True)[0]
...
def compute_residual(network, input_values):
    ...
    grad_u = torch.autograd.grad(u, input_values, grad_outputs=torch.ones(input_values.shape[0], ),
                                 create_graph=True)[0]
    grad_u_t = grad_u[:, 0]
    grad_u_x = grad_u[:, 1]
    grad_grad_u_x = torch.autograd.grad(grad_u_x, input_values, grad_outputs=torch.ones(input_values.shape[0]),
                                        create_graph=True)[0]
